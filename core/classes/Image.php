...
class Image
{
	...
	private function imagefile($image, $path)
	{
		...
		switch ($this->obj)
		{
			case 'jpg' : return imagejpeg($image, $path, $config['imgquality']); break;
			case 'jpeg': return imagejpeg($image, $path, $config['imgquality']); break;
	...
	private function imageprint($image)
	{
		...
		switch ($this->obj)
		{
			case 'jpg' : return imagejpeg($image, NULL, $config['imgquality']); break;
			case 'jpeg': return imagejpeg($image, NULL, $config['imgquality']); break;
	...
	public function createwater($path, $water = FALSE, $type = FALSE, $text = FALSE)
	{
		...
		if (empty($type))
		{
			...
			$this->_imagecopymerge($scrimg, $insert, $dest_x, $dest_y, 0, 0, $insert_x, $insert_y, $config['markquality']);
		}
		else
		{
			$scrimg = $this->makeimage($path);
	...
	public function imgconvert($inimg, $outimg, $type = FALSE)
	{
		...
		imagejpeg($scrimg, $outimg, $config['imgquality']);
		imagedestroy($scrimg);
		unlink($inimg);
		$this->obj = '';
	}
	...
	public function url_thumb($url, $patch, $width = null, $height = null)
	{
		...
		$image = $this->urlimg($url, $config['maxfile']);
		...
		if ($tmp_name AND copy($tmp_name, $patch.$name.$this->ext))
		{
			...
			$this->createthumb($patch.$image, $patch, $image_thumb, $width, $height, $config['resize']);
