...
class userapi
{
	...
	function avatar($type, $scr, $user, $link = FALSE)
	{
		$avatar = '';

		if ($scr)
		{
			if ($type == 'avatar.driver.upload') {
				$avatar = ($link) ? '<img src="'.$this->data['forumpath'].'download/file.php?avatar='.$scr.'" alt="'.$user.'" />' : $this->data['forumpath'].'download/file.php?avatar='.$scr;
			} elseif ($type == 'avatar.driver.remote') {
				$avatar = ($link) ? '<img src="'.$scr.'" alt="'.$user.'" />' : $scr;
			} elseif ($type == 'avatar.driver.gallery') {
				$avatar = ($link) ? '<img src="'.$this->data['avatargalpath'].$scr.'" alt="'.$user.'" />' : $this->data['avatargalpath'].$scr;
			}
		}
					...
					if (intval($newuser['ban_id']) == 0 AND intval($newuser['user_id']) > 1)
					{
						...
						$this->usermain = array
							(
								...
								'avatar'    => $this->avatar($newuser['user_avatar_type'], $newuser['user_avatar'], $newuser['username'], TRUE)
							);
					}
				}
			}
			...
			while ($item = $this->db->fetchassoc($inq))
			{
				$associat[$item['user_id']] = array
					(
						...
						'avatar'  => $this->avatar($item['user_avatar_type'], $item['user_avatar'], $newuser['username'], FALSE),
						'phone'   => '',
						'city'    => '',
						'skype'   => ''
						'www'     => ''
