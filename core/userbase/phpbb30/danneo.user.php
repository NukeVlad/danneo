<?php
/**
 * File:        /core/userbase/phpbb30/danneo.user.php
 *
...
class userapi
{
	...
	{
		$this->db->query("UPDATE ".$this->data['prefix']."users SET user_website = '".$this->db->escape($www)."' WHERE user_id = '".intval($this->usermain['userid'])."'");
	}
	...
	function associat($in = FALSE)
	{
		...
		if ($in)
		{
			$inq = $this->db->query
					(
						"SELECT user_id, username, user_regdate, user_avatar, user_avatar_type, user_website
						 FROM ".$this->data['prefix']."users
	...
	function usersave($uid, $edit)
	{
		...
		{
			if (isset($edit['mail']) AND !empty($edit['mail']) AND $this->db->numrows($this->db->query("SELECT user_id FROM ".$this->data['prefix']."users WHERE user_email = '".$this->db->escape($edit['mail'])."' AND user_id <> $uid")) == 0)
			{
