<?php
/**
 * File:        /admin/mod/photos/index.php
 *
...
if ($ADMIN_AUTH == 1 AND $sess['hash'] == $ops)
{
	...
	if ($ADMIN_PERM == 1 OR in_array($ADMIN_ID, $CHECK_ADMIN['admid']))
	{
		...
		$legaltodo = array
			(
				'index', 'optsave', 'cat', 'catdel', 'catup',
				'catadd', 'cataddsave', 'catedit', 'cateditsave',
				'work', 'arrdel', 'arrmove', 'arract', 'arracc',
				'list', 'add', 'addsave', 'edit', 'editsave', 'thumb', 'act', 'del',
				'new', 'newadd', 'newdel', 'newarrdel',
				'upmass', 'massupsave', 'ajaxedittitle',
				'comment', 'commentrep', 'commentedit', 'commenteditrep',
				'ajaxsavetitle', 'ajaxeditcat', 'ajaxsavecat', 'ajaxeditdate', 'ajaxsavedate', 'thumb', 'videoview',
		...
		function this_menu()
		{
			...
			$link = '<a'.cho('index').' href="index.php?dn=index&amp;ops='.$sess['hash'].'">'.$lang['all_set'].'</a>'
					.'<a'.cho('list').' href="index.php?dn=list&amp;ops='.$sess['hash'].'">'.$lang['all_photos'].'</a>'
					...
					.'<a'.cho('add').' href="index.php?dn=add&amp;ops='.$sess['hash'].'">'.$lang['add_photos'].'</a>'
		...
		if ($_REQUEST['dn'] == 'index')
		{
			...
			while ($itemset = $db->fetchrow($inqset))
			{
				...
				{
					echo	in_array($itemset['setname'], array('catmain', 'addit', 'comact', 'rating', 'rec', 'rss')) ? '<tr><th colspan="2"></th></tr>' : '';
					echo '	<tr>
								<td class="first">
		...
		if ($_REQUEST['dn'] == 'catedit')
		{
			...
			echo '	<div class="section">
					<form action="index.php" method="post">
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['cat_edit'].'</caption>
						<tr>
							<td class="first"><span>*</span> '.$lang['all_name'].'</td>
							<td>
		...
		if ($_REQUEST['dn'] == 'list')
		{
			global $PLATFORM, $realmod, $selective, $catid, $nu, $p, $cat, $s, $l, $ajax, $filter, $fid;
			...
			$myfilter = array(
				...
				'access' => array('acc', 'all_access', 'type', array('unimportant', 'all_all', 'all_user_only'), array('', 'all', 'user')),
			...
			echo '	<div class="section">
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['photo_video'].'</caption>
						<tr>
							<td class="vm">
								'.$lang['all_cat_one'].':&nbsp;
								<form action="index.php" method="post">
					...
					<table id="list" class="work">
						...
						<tr>
							...
							<th class="work-no-sort">'.$lang['all_thumb'].'</th>
			...
			while ($item = $db->fetchrow($inq))
			{
				...
				// Prevew
				$prevew = ( ! empty($item['image'])) ? $conf['site_url'].'/'.$item['image'] : ADMPATH.'/template/images/donot.jpg';
				...
				if ($ajaxlink == 1) {
					echo '		<div id="te'.$item['id'].'">
									<a href="javascript:$.ajaxeditor(\'index.php?dn=ajaxedittitle&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'\',\'te'.$item['id'].'\',\'405\')" title="'.$lang['all_change'].'">'.preparse_un($item['title']).'</a>
								</div>';
				} else {
				...
				if ($ajaxlink == 1) {
					echo '		<div id="ce'.$item['id'].'" class="cats">
									<a class="notooltip" href="javascript:$.ajaxeditor(\'index.php?dn=ajaxeditcat&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'\',\'ce'.$item['id'].'\',\'305\')" title="'.$lang['all_change'].'">'.preparse_un(linecat($item['catid'],$catcaches)).'</a>
								</div>';
				} else {
				...
				if ($ajaxlink == 1) {
					echo '		<div id="de'.$item['id'].'">
									<a href="javascript:$.ajaxeditor(\'index.php?dn=ajaxeditdate&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'\',\'de'.$item['id'].'\',\'220\')" title="'.$lang['all_change'].'">'.format_time($item['public'],0,1).'</a>
								</div>';
				} else {
				...
				echo '		</td>
							<td class="'.$style.' pw5">
								<a class="photo-view" href="'.$prevew.'" title="'.$lang['file_view'].'"><img src="index.php?dn=thumb&amp;id='.$item['id'].'&amp;x=36&amp;h=27&amp;r=yes&amp;ops='.$sess['hash'].'" alt="" /></a>
							</td>
		...
		if ($_REQUEST['dn']=='add')
		{
			...
			$inqtag = $db->query("SELECT * FROM ".$basepref."_".PERMISS."_tag");
			...
			echo '	<div class="section">
					...
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['add_photos'].'</caption>
						<tr>
			...
			echo '			</td>
						...
						<tr><th></th><th class="site">'.$lang['all_image'].'</th></tr>
						<tr>
							...
							<td>
								...
								<input class="side-button" onclick="javascript:$.quickupload(\''.$sess['hash'].'&amp;objdir=/'.PERMISS.'/album/\')" value="'.$lang['file_review'].'" type="button">
							</td>
						</tr>
						<tr>
			...
			if($conf[PERMISS]['tags'] == 'yes')
			{
				...
				echo '	<tr><th></th><th class="site">'.$lang['options'].'</th></tr>
						<tr>
		...
		{
			global	$catid, $public, $stpublic, $unpublic, $title, $subtitle, $cpu, $customs, $keywords, $descript, $text, $hits,
					$image, $image_thumb, $image_alt, $act, $acc, $group, $tagword, $author, $imp, $new;
			...
			$customs = ( ! empty($customs)) ? preparse($customs, THIS_TRIM) : $title;
			$descript = preparse($descript, THIS_TRIM);
			$keywords = preparse($keywords, THIS_TRIM);

			$file_type = array('gif', 'jpg', 'jpeg', 'png', 'webp');

			$ext_thumb = strtolower(pathinfo($image_thumb, PATHINFO_EXTENSION));
			$valid_thumb = ( ! in_array($ext_thumb, $file_type)) ? 1 : 0;

			$ext_image = strtolower(pathinfo($image, PATHINFO_EXTENSION));
			$valid_image = ( ! in_array($ext_image, $file_type)) ? 1 : 0;

			if ($valid_thumb OR $valid_image)
			{
				$tm->header();
				$tm->error($modname[PERMISS], $lang['add_photos'], $lang['error_type']);
				$tm->footer();
			}
			...
			if (
				...
				preparse($image, THIS_EMPTY) == 1
			) {
				$tm->header();
				$tm->error($modname[PERMISS], $lang['add_photos'], $lang['pole_add_error']);
				$tm->footer();
			}
			else
			{
				...
				{
					$tm->header();
					$tm->error($modname[PERMISS], $lang['add_photos'], $lang['cpu_error_isset'], $title);
					$tm->footer();
				}
			}
			...
			$hits = ( ! empty($hits)) ? preparse($hits, THIS_INT) : 0;
			$catid = preparse($catid, THIS_INT);
		...
		{
			global $selective, $catid, $id, $p, $cat, $nu, $s, $l;
			...
			if ($_REQUEST['dn'] == 'newadd')
			{
				...
				$author = '&#8212;';
				$item['keywords'] = $item['tags'] = null;
				$item['hits'] = $item['trans'] = $item['imp'] = 0;

				$item['customs'] = $item['descript'] = $item['image_alt'] = $item['title'];
				$item['cpu'] = cpu_translit($item['title']);
			...
			$caption = ($_REQUEST['dn'] == 'newadd') ? $lang['add_photos'] : $lang['edit_photos'];
			...
			echo '	<div class="section">
					...
					<table class="work">
						...
						<tr>
							<td>'.$lang['sub_title'].'</td>
							<td><input type="text" name="subtitle" size="70" value="'.preparse_un($subtitle).'"> <span class="light">&lt;h2&gt;</span></td>
						</tr>
						</tbody>
						<tbody class="tab-1">';
			if ($conf['cpu'] == 'yes')
			...
			{
				echo '	<tr>
							<td class="first"><span>*</span> '.$lang['in_gal'].'</td>
							<td>
								<select name="catid" class="sw250">';
				$catcache = array();
				$catid = $item['catid'];
			...
			echo '			</td>
						...
						<tr>
							<td class="first"><span>*</span> '.$lang['all_image'].'</td>
							<td>
								<input name="image" id="image" size="70" type="text" value="'.$item['image'].'" required="required">
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/\',\'&amp;field[1]=image&amp;field[2]=image_thumb\')" value="'.$lang['filebrowser'].'" type="button">
							</td>
						</tr>
						<tr>
							<td>'.$lang['all_alt_image'].'</td>
							<td><input name="image_alt" size="70" type="text" value="'.preparse_un($item['image_alt']).'"></td>
						</tr>
						</tbody>
						<tbody class="tab-2">';
			if($conf[PERMISS]['tags'] == 'yes')
			...
			echo '		<tr><th></th><th class="site">'.$lang['all_info'].'</th></tr>
						<tr>
							<td>'.$lang['all_hits'].'</td>
		...
		{
			global	$id, $catid, $public, $stpublic, $unpublic, $title, $subtitle, $cpu, $customs, $keywords, $descript, $text, $hits,
					$image, $image_thumb, $image_alt, $act, $acc, $group, $tagword, $author, $imp, $p, $cat, $nu, $s, $l, $c;
			...
			{
				$tm->header();
				$tm->error($modname[PERMISS], $c, $lang['error_type']);
				$tm->footer();
			}
			...
			if (
				...
				$tm->header();
				$tm->error($modname[PERMISS], $c, $lang['pole_add_error']);
				$tm->footer();
			}
			else
			{
				...
				{
					$tm->header();
					$tm->error($modname[PERMISS], $c, $lang['cpu_error_isset'], $title);
					$tm->footer();
				}
			}
		...
		if ($_REQUEST['dn'] == 'del')
		{
			...
			else
			{
				...
				$tm->header();
				$tm->shortdel($modname[PERMISS], $lang['del_photo'], $yes, $not, preparse_un($item['title']));
				$tm->footer();
			}
		}
		...
		if ($_REQUEST['dn'] == 'upmass')
		{
			...
			$inquiry = $db->query("SELECT catid, parentid, catname FROM ".$basepref."_".PERMISS."_cat ORDER BY posit ASC");
			$catcache = array();
			while ($citem = $db->fetchrow($inquiry)) {
		...
		if ($_REQUEST['dn'] == 'massupsave')
		{
			global	$conf, $catid, $thumb, $width, $height, $width_big, $height_big,
					$resize_big, $injpg, $wmark, $resize, $names, $desc, $files, $group, $acc;
			...
			$catid = preparse($catid, THIS_INT);
			$ext   = array('gif', 'jpeg', 'jpg', 'png', 'webp');
		...
		if ($_REQUEST['dn'] == 'new')
		{
			...
			echo '	<div class="section">
					<form action="index.php" method="post">
					<table id="list" class="work">
						<caption>'.$modname[PERMISS].': '.$lang['addit_photos'].'</caption>';
			if ($db->numrows($inq) > 0)
			{
				echo '	<tr><td colspan="7">'.$amount.'</td></tr>
						<tr>
							<th class="work-no-sort pw20 al">'.$lang['all_name'].'</th>
							<th class="work-no-sort">'.$lang['all_cat_one'].'</th>
							<th class="work-no-sort">'.$lang['author'].'</th>
							<th class="work-no-sort">'.$lang['photos_data'].'</th>
							<th class="work-no-sort">'.$lang['all_thumb'].'</th>
							<th class="work-no-sort">'.$lang['sys_manage'].'</th>
				...
				while ($item = $db->fetchrow($inq))
				{
					...
					echo '	<tr class="list">
								<td class="al">
									<a href="index.php?dn=newadd&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'" title="'.$lang['all_edit'].'&nbsp; &#8260; &nbsp;'.$lang['all_add'].'">'.preparse_un($item['title']).'</a>
								</td>
								<td>'.preparse_un(linecat($item['catid'], $catcaches)).'</td>
								<td>'.$author.'</td>
								<td>'.format_time($item['public'], 1, 1).'</td>
								<td class="pw5">
									<a class="photo-view" href="'.$prevew.'" title="'.$lang['file_view'].'"><img src="index.php?dn=thumb&amp;id='.$item['id'].'&amp;x=36&amp;h=27&amp;r=yes&amp;us=yes&amp;ops='.$sess['hash'].'" alt="" /></a>
								</td>
								<td class="gov">
		...
		/**
		 * Массовое удаление добавленных фото
		 --------------------------------------*/
		if ($_REQUEST['dn'] == 'newarrdel')
		{
			...
			if (preparse($array, THIS_ARRAY) == 1)
			{
				if ($ok == 'yes')
				{
					allarrdel($array, 'id', PERMISS.'_user', 0, 1); // 1 : del image
		...
		/**
		 * Комментарии отдельного фото (сохранение)
		 -------------------------------------------*/
		...
		/**
		 * Комментарии - Все фото (сохранение)
		 ---------------------------------------*/
		...
		/**
		 * Быстрое редактирование названия изображения
		 -------------------------------------------*/
		if ($_REQUEST['dn'] == 'ajaxedittitle')
		{
			...
			echo '	<form action="index.php" method="post" id="post" name="post" onsubmit="return $.posteditor(this,\'te'.$item['id'].'\',\'index.php?dn=ajaxsavetitle&id='.$item['id'].'&ops='.$sess['hash'].'\')">
					<div style="width: 400px;">
						<input type="text" name="title" size="60" maxlength="255" value="'.preparse_un($item['title']).'">&nbsp;
						<input type="hidden" name="ops" value="'.$sess['hash'].'">
		...
		/**
		 * Быстрое редактирование названия изображения (сохранение)
		 -----------------------------------------------------------*/
		...
		/**
		 * Быстрое изменение категории изображения
		 -------------------------------------------*/
		...
		/**
		 * Быстрое изменение категории изображения (сохранение)
		 --------------------------------------------------------*/
		...
		/**
		 * Быстрое изменение даты изображения
		 -------------------------------------*/
		...
		/**
		 * Быстрое изменение даты изображения (сохранение)
		 ---------------------------------------------------*/
		...
		if ($_REQUEST['dn'] == 'thumb')
		{
			...
			$id = preparse($id, THIS_INT);
			$SQL = isset($us) ? '_user' : '';
			$item = $db->fetchrow($db->query("SELECT image, image_thumb FROM ".$basepref."_".PERMISS."".$SQL." WHERE id = '".$id."'"));
			thumb($item['image_thumb'], $x, $h, $r);
			exit();
		}
