<?php
/**
 * File:        /admin/mod/article/index.php
 *
...
{
	global $ADMIN_ID, $CHECK_ADMIN, $AJAX, $db, $basepref, $tm, $conf, $wysiwyg, $modname, $lang, $sess, $ops, $cache, $letter;
	...
	if ($ADMIN_PERM == 1 OR in_array($ADMIN_ID, $CHECK_ADMIN['admid']))
	{
		...
		$legaltodo = array
			(
				'index', 'optsave', 'cat', 'catadd', 'catedit', 'catdel', 'cataddsave', 'catup', 'cateditsave',
				'list', 'work', 'arrdel', 'arrmove', 'arract', 'arracc',
				'add', 'edit', 'addsave', 'editsave', 'act', 'del',
				'new', 'newadd', 'newdel', 'newarrdel',
				'comment', 'commentrep', 'commentedit', 'commenteditrep',
				'mediaadd', 'mediaaddsave',
		...
		function this_menu()
		{
			...
			$link = '<a'.cho('index').' href="index.php?dn=index&amp;ops='.$sess['hash'].'">'.$lang['all_set'].'</a>'
					.'<a'.cho('list').' href="index.php?dn=list&amp;ops='.$sess['hash'].'">'.$lang['all_publication'].'</a>'
					.'<a'.cho('add, newadd').' href="index.php?dn=add&amp;ops='.$sess['hash'].'">'.$lang['add_publication'].'</a>'
			...
			if (isset($conf[PERMISS]['addit']) AND $conf[PERMISS]['addit'] == 'yes')
			{
				...
				$link.= '<a'.cho('new').' href="index.php?dn=new&amp;ops='.$sess['hash'].'">'.$lang['added'].'&nbsp; &#8260; &nbsp;'.$c['total'].'</a>';
			}
		...
		if ($_REQUEST['dn'] == 'index')
		{
			...
			while ($itemset = $db->fetchrow($inqset))
			{
				...
				{
					echo	in_array($itemset['setname'], array('cats', 'rec', 'comact', 'addit', 'rss', 'rating')) ? '<tr><th colspan="2"></th></tr>' : '';
					echo '	<tr>
		...
		if ($_REQUEST['dn'] == 'list')
		{
			global $db, $conf, $ro, $realmod, $selective, $catid, $nu, $p, $cat, $s, $l, $ajax, $filter, $fid;
			...
			$sort = array('id', 'title', 'public', 'uppublic', 'hits', 'comments', 'author');
			...
			echo '	<div class="section">
					<table id="list" class="work">
						<caption>'.$modname[PERMISS].': '.$lang['all_publication'].'</caption>
						<tr>
							<td class="vm">
								'.$lang['all_cat_one'].':&nbsp;
								<form action="index.php" method="post">
					...
					<table id="list" class="work">
						...
						<tr>
							...
							<th'.listsort('uppublic').'>'.$lang['all_updata'].'</th>
			...
			while ($item = $db->fetchrow($inq))
			{
				...
				echo '			<a href="index.php?dn=del&amp;p='.$p.'&amp;nu='.$nu.$link.'&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'"><img src="'.ADMPATH.'/template/skin/'.$sess['skin'].'/images/del.png" alt="'.$lang['all_delet'].'" /></a>
							...
							<td class="'.$style.' mark">
								<input id="check-box" type="checkbox" name="array['.$item['id'].']" value="yes" />
							</td>
						</tr>';
			}
			echo '		<tr>
			...
			echo '				<input type="hidden" name="dn" value="work" />
						...
						<tr><td class="vm" colspan="10">'.$pages.'</td></tr>
					</table>
					</form>
					</div>';
		...
		if ($_REQUEST['dn'] == 'add')
		{
			...
			echo '	<div class="section">
					...
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['add_publication'].'</caption>
						<tr>
						...
						<tr>
							<td class="first"><span>*</span> '.$lang['all_name'].'</td>
							<td>
								<input type="text" name="title" id="title" size="70" /> <span class="light">&lt;h1&gt;</span>
							</td>
						</tr>
						</tbody>
						<tbody class="tab-2">
			...
			if($conf[PERMISS]['tags'] == 'yes')
			{
				...
				echo '	<tr><th></th><th class="site">'.$lang['contents'].'</th></tr>
						<tr>
			...
			echo '			</td>
						...
						<tr>
							...
							<td class="usewys">
								<div class="infosexample">
									<a id="pages-hint" class="infos-link" href="#">'.$lang['all_int_pages'].'</a>
									<div id="pageshint" style="display: none;">'.$lang['all_int_pages_hiht'].'</div>
								</div>';
			if ($wysiwyg == 'yes')
			{
				$WYSFORM = 'textmore';
						...
						<tr>
							...
							<td class="vm">
								...
								<div>
									...
									<a class="side-button" href="javascript:$.personalupload(\''.$sess['hash'].'&amp;objdir=/'.PERMISS.'/image/\');">'.$lang['file_upload'].'</a>
								</div>
							</td>
						</tr>
			...
			if (isset($conf['user']['regtype']) AND $conf['user']['regtype'] == 'yes') {
				echo '	<tr>
							...
							<td>';
								$tm->textarea('textnotice', 3, 70, '', true, false, 'ignorewysywig');
				echo '		</td>
						</tr>';
			}
				echo '	<tr>
						...
						<tr>
							<td>'.$lang['all_hits'].'</td>
							<td><input type="text" name="hits" size="25" maxlength="10" /></td>
						</tr>';
			echo '		<tr><th></th><th class="site">'.$lang['all_video'].'</th></tr>
						<tr>
							<td>'.$lang['locale_file'].'</td>
							<td>
								<input name="video" id="video" size="70" type="text" />
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/video/\',\'&amp;field[1]=video\')" value="'.$lang['filebrowser'].'" type="button" />
							</td>
						</tr>
						<tr>
							<td>'.$lang['all_video'].'&nbsp; &#8260; &nbsp;YouTube</td>
							<td><input name="youtu" size="70" type="text" />';
								$tm->outhint('http://youtu.be/<br>http://www.youtube.com/watch?v=');
			echo '			</td>
						</tr>
						</tbody>
						<tbody class="tab-1">
		...
		/**
		 * Добавление статьи (сохранение)
		 ------------------------------------*/
		if ($_REQUEST['dn'] == 'addsave')
		{
			global	$catid, $public, $stpublic, $unpublic, $title, $subtitle, $cpu, $customs, $descript, $keywords, $textshort, $textmore, $hits,
					$video, $youtu, $image, $image_thumb, $image_align, $image_alt, $textnotice, $acc, $group, $act, $tagword, $images,
					$letid, $letter, $author, $imp, $files, $facc, $fgroups, $id, $new, $p, $nu;
			...
			{
				$tm->header();
				$tm->error($modname[PERMISS], $lang['add_publication'], $lang['pole_add_error']);
				$tm->footer();
			}
			else
			{
				...
				{
					$tm->header();
					$tm->error($modname[PERMISS], $lang['add_publication'], $lang['cpu_error_isset'], $title);
					$tm->footer();
				}
			}
			...
			if ( ! empty($video))
			{
				$typevideo = array('flv', 'mp4', 'webm', 'ogv');
				$extvideo = strtolower(pathinfo($video, PATHINFO_EXTENSION));
				$validvideo = (in_array($extvideo, $typevideo)) ? 1 : 0;

				if ($validvideo == 0)
				{
					$tm->header();
					$tm->error($modname[PERMISS], $lang['add_publication'], $lang['error_type']);
					$tm->footer();
				}
			}
			...
			if ( ! empty($youtu))
			{
				preg_match('~(http|https)://(?:www\.)?(?:youtube.com|youtu.be)/(?:watch\?)?(?:.*?)((?<=v=|/)[0-9a-z-_]{11})(?:.*?)~is', $youtu, $match);
				...
				{
					$tm->header();
					$tm->error($modname[PERMISS], $lang['add_publication'], $lang['error_youtu']);
					$tm->footer();
				}
			}
			...
			$youtu = preparse($youtu, THIS_TRIM, 0, 255);
			...
			$group = (isset($conf['user']['groupact']) AND $conf['user']['groupact'] == 'yes' AND $acc == 'group' AND is_array($group)) ? Json::encode($group) : '';
			$acc = ($acc == 'user' OR $acc == 'group') ? 'user' : 'all';

			$fgroups = (isset($conf['user']['groupact']) AND $conf['user']['groupact'] == 'yes' AND $facc == 'group' AND is_array($fgroups)) ? Json::encode($fgroups) : '';
		...
		if ($_REQUEST['dn'] == 'edit')
		{
			global $catid, $selective, $id, $p, $nu, $s, $l, $fid;
			...
			echo '	<div class="section">
					<form action="index.php" method="post" id="total-form">
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['edit_public'].'</caption>
						<tr>
							<th class="ar site">'.$lang['all_bookmark'].' &nbsp; </th>
							<th>'.$tabs.'</th>
						</tr>
			...
			{
				$tm->textarea('textmore', 10, 70, $item['textmore'], (($wysiwyg == 'yes') ? 0 : 1));
			}
			echo '			</td>
						</tr>
						</tbody>
			...
			echo '		<tr>
							<td>'.$lang['author'].'</td>
							<td><input type="text" name="author" size="25" value="'.$item['author'].'" /></td>
						</tr>
						<tr>
							<td>'.$lang['all_hits'].'</td>
							<td><input type="text" name="hits" size="25" maxlength="10" value="'.$item['hits'].'" /></td>
						</tr>
						<tr><th></th><th class="site">'.$lang['all_video'].'</th></tr>
						<tr>
							<td>'.$lang['locale_file'].'</td>
							<td>
						...
						<tr>
							<td>'.$lang['all_video'].'&nbsp; &#8260; &nbsp;YouTube</td>
							<td>
								<input name="youtu" size="70" type="text" value="'.(( ! empty($item['youtu'])) ? 'http://youtu.be/'.$item['youtu'] : '').'" />';
								$tm->outhint('http://youtu.be/<br>http://www.youtube.com/watch?v=');
			echo '				'.( ! empty($item['youtu']) ? ' <a href="http://www.youtube.com/watch?v='.$item['youtu'].'" target="_blank"><img src="'.ADMPATH.'/template/images/blank.png" alt="'.$lang['all_chek'].'" /></a>' : '');
			echo '			</td>
						</tr>
						</tbody>
						<tbody class="tab-1">
		...
		{
			global	$id, $catid, $public, $stpublic, $unpublic, $uppublic, $title, $subtitle, $cpu, $customs, $descript, $keywords, $textshort, $textmore, $hits,
					$video, $youtu, $image, $image_thumb, $image_align, $image_alt, $textnotice, $acc, $group, $act, $tagword, $images,
					$letid, $letter, $author, $imp, $p, $cat, $nu, $s, $l, $fid, $files, $facc, $fgroups;
			...
			if (
				...
				$tm->header();
				$tm->error($modname[PERMISS], $lang['edit_public'], $lang['pole_add_error']);
				$tm->footer();
			}
			else
			{
				...
				{
					$tm->header();
					$tm->error($modname[PERMISS], $lang['edit_public'], $lang['cpu_error_isset'], $title);
					$tm->footer();
				}
			}
			...
			if ( ! empty($video))
			{
				...
				{
					$tm->header();
					$tm->error($lang['all_edit'], $title, $lang['error_type']);
					$tm->footer();
				}
			}
			...
			if ( ! empty($youtu))
			{
				...
				{
					$tm->header();
					$tm->error($lang['all_edit'], $title, $lang['error_youtu']);
					$tm->footer();
				}
			}
			...
			$db->query
				(
					"UPDATE ".$basepref."_".PERMISS." SET
					 ...
					 youtu       = '".$db->escape($youtu)."',
					 hits        = '".$hits."',
					 acc         = '".$acc."',
					 act         = '".$act."',
					 groups      = '".$db->escape($group)."',
					 comments    = '".intval($count['total'])."',
					 tags        = '".$tags."',
		...
		if ($_REQUEST['dn'] == 'del')
		{
			...
			else
			{
				...
				$tm->header();
				$tm->shortdel($modname[PERMISS], $lang['del_public'], $yes, $not, preparse_un($item['title']));
				$tm->footer();
			}
		}

		/**
		 * Комментарии отдельной статьи
		 ----------------------------------*/
						...
						$(function()
						{
							...
							$(".submit").colorbox({
								onLoad: function() {
									...
									"href"  : $.apanel + "/mod/'.PERMISS.'/index.php?dn=commentedit&p='.$p.'&id='.$id.'&ops='.$sess['hash'].'&ajax=1&t='.time().'"
							});
						});
						</script>';
			}
			...
			echo '	<div id="fb-work-comm">
					<table class="fb-work">
						<tr>
							...
							<th class="ac">'.$lang['comment_text'].'</th>
		...
		/**
		 * Комментарии отдельной статьи (сохранение)
		 ----------------------------------------------*/
		...
		/**
		 * Комментарии - Все статьи (сохранение)
		 ------------------------------------------*/
		...
		if ($_REQUEST['dn'] == 'new')
		{
			...
			echo '	<div class="section">
					<form action="index.php" method="post">
					<table id="list" class="work">
						<caption>'.$modname[PERMISS].': '.$lang['added_public'].'</caption>';
			if ($db->numrows($inq) > 0)
			{
				...
				while ($item = $db->fetchrow($inq))
				{
					...
					if ( ! empty($item['userid']))
					{
						if (in_array('user', $realmod))
						{
							...
							if ( ! empty($udata))
							{
								$author = '<a href="'.$conf['site_url'].$ro->seo($userapi->data['linkprofile'].$item['userid']).'" title="'.$lang['profile'].' - '.$author.'" target="_blank">'.$udata['uname'].'</a>';
							}
						}
					}

					echo '	<tr class="list">
								<td class="site al"><a href="index.php?dn=newadd&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'" title="'.$lang['all_edit'].'&nbsp; &#8260; &nbsp;'.$lang['all_add'].'">'.preparse_un($item['title']).'</a></td>
		...
		if ($_REQUEST['dn'] == 'newdel')
		{
			...
			else
			{
				...
				$tm->header();
				$tm->shortdel($modname[PERMISS].'&nbsp; &#8260; &nbsp;'.$lang['added'], $lang['del_public'], $yes, $not, preparse_un($item['title']));
				$tm->footer();
			}
		}

		/**
		 * Массовое удаление добавленных статей
		 -------------------------------------------*/
			...
			if (preparse($array, THIS_ARRAY) == 1)
			{
				...
				else
				{
					...
					echo '	<div class="section">
							<form action="index.php" method="post">
							<table id="arr-work" class="work">
								<caption>'.$modname[PERMISS].'&nbsp; &#8260; &nbsp;'.$lang['added'].': '.$lang['all_delet'].' ('.$count.')</caption>
								<tr>
									<td class="cont">'.$lang['alertdel'].'</td>
								</tr>
								<tr class="tfoot">
		...
		/**
		 * Модерировать добавленную статью
		 ------------------------------------*/
		if ($_REQUEST['dn'] == 'newadd')
		{
			...
			$item = $db->fetchrow($db->query("SELECT * FROM ".$basepref."_".PERMISS."_user WHERE id = '".$id."'"));
			$time = CalendarFormat($item['public']);
			$iamge_alt = ( ! empty($item['image_thumb'])) ? $item['title'] : '';
			...
			echo '	<div class="section">
					<form action="index.php" method="post" id="total-form">
					<table class="work">
						<caption>'.$modname[PERMISS].': '.$lang['moder_public'].'</caption>
						<tr>
							<th class="ar site">'.$lang['all_bookmark'].' &nbsp; </th>
							<th>'.$tabs.'</th>
						</tr>
						...
						<tr>
							<td class="first"><span>*</span> '.$lang['all_name'].'</td>
							<td>
								<input type="text" name="title" id="title" size="70" value="'.preparse_un($item['title']).'" required="required" />
							</td>
						</tr>
						</tbody>
						<tbody class="tab-2">
			...
			if ($conf['cpu'] == 'yes')
			{
				echo '	<tr>
							<td>'.$lang['all_cpu'].'</td>
							<td>
								<input type="text" name="cpu" id="cpu" size="70" value="'.cpu_translit($item['title']).'" />';
								$tm->outtranslit('title', 'cpu', $lang['cpu_int_hint']);
				echo '		</td>
						</tr>';
			}
			echo '		<tr>
							<td>'.$lang['custom_title'].'</td>
							<td><input type="text" name="customs" size="70" value="'.preparse_un($item['title']).'" /></td>
						</tr>
						</tbody>
						<tbody class="tab-2">
						<tr>
							<td>'.$lang['all_descript'].'</td>
							<td><input type="text" name="descript" size="70" value="'.preparse_un($item['title']).'" /></td>
						</tr>
						<tr>
							<td>'.$lang['all_keywords'].'</td>
							<td>
			...
			echo '			</td>
						...
						<tr>
							<td>'.$lang['all_data'].'</td>
							<td>
								<input type="text" name="public" id="public" value="'.$time.'" />';
								Calendar('cal', 'public');
			echo '			</td>
						</tr>
						<tr>
							<td>'.$lang['all_stpublic'].'</td>
							<td>
								<input type="text" name="stpublic" id="stpublic" />';
								Calendar('stcal', 'stpublic');
			echo '			</td>
						</tr>
						<tr>
							<td>'.$lang['all_unpublic'].'</td>
							<td>
							<input type="text" name="unpublic" id="unpublic">';
								Calendar('uncal', 'unpublic');
			echo '			</td>
						</tr>
						</tbody>
						...
						<td>'.$lang['all_in_cat'].'</td>
							<td>
								<select name="catid" class="sw250">
									<option value="0"> &#8212; '.$lang['cat_not'].' &#8212; </option>';
			$inq = $db->query("SELECT catid, parentid, catname FROM ".$basepref."_".PERMISS."_cat ORDER BY posit ASC");
			$catcache = array();
			$catid = $item['catid'];
			...
			if ($conf[PERMISS]['tags'] == 'yes')
			{
				echo '	<tr>
							...
							<td>
								<div id="tag-area"></div>
								<div style="margin-top: 5px;"><a class="side-button" href="javascript:$.addtaginput(\'total-form\',\'tag-area\',\''.$sess['hash'].'\');">'.$lang['all_submint'].'</a></div>
							</td>
						</tr>';
			}
			echo '		<tr>
						...
						<tr>
							...
							<td>
								...
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/add/\',\'&amp;field[1]=image_thumb&amp;field[2]=image&amp;field[3]=video\')" value="'.$lang['filebrowser'].'" type="button" />
								<input class="side-button" onclick="javascript:$.quickupload(\''.$sess['hash'].'&amp;objdir=/'.PERMISS.'/add/\')" value="'.$lang['file_review'].'" type="button" />
							</td>
						</tr>
						</tbody>
						<tbody class="tab-2">
						<tr>
							...
							<td>
								...
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/add/\',\'&amp;field[1]=image&amp;field[2]=image_thumb&amp;field[3]=video\')" value="'.$lang['filebrowser'].'" type="button" />
							</td>
						</tr>
						</tbody>
						<tbody class="tab-1">
						<tr>
							<td>'.$lang['all_alt_image'].'</td>
							<td><input name="image_alt" id="image_alt" value="'.$iamge_alt.'" size="70" type="text" /></td>
						</tr>
						</tbody>
						<tbody class="tab-2">
						<tr>
			...
			if (isset($conf['user']['regtype']) AND $conf['user']['regtype'] == 'yes')
			{
				...
				if ($conf['user']['groupact'] == 'yes')
				{
					...
					while ($items = $db->fetchrow($inqs))
					{
						echo '		<input type="checkbox" name="group['.$items['gid'].']" value="yes" /><span>'.$items['title'].'</span>';
					}
				}
				echo '			</div>
							</td>
		...
		/**
		 * Быстрое редактирование названия статьи
		 -------------------------------------------*/
		...
		/**
		 * Быстрое редактирование названия статьи (сохранение)
		 -------------------------------------------------------*/
		...
		/**
		 * Быстрое изменение категории статьи
		 ----------------------------------------*/
		...
		/**
		 * Быстрое изменение категории статьи (сохранение)
		 ----------------------------------------------------*/
		...
		/**
		 * Быстрое изменение даты статьи (сохранение)
		 -----------------------------------------------*/
		...
		if ($_REQUEST['dn'] == 'tag')
		{
			...
			while ($item = $db->fetchrow($inq))
			{
				...
				echo '		</td>
							...
							<td class="gov">
								<a class="edit" href="index.php?dn=tagedit&amp;p='.$p.'&amp;nu='.$nu.'&amp;tagid='.$item['tagid'].'&amp;ops='.$sess['hash'].'"><img src="'.ADMPATH.'/template/skin/'.$sess['skin'].'/images/edit.png" alt="'.$lang['all_edit'].'" /></a>
								<a class="del" href="index.php?dn=tagdel&amp;p='.$p.'&amp;nu='.$nu.'&amp;tagid='.$item['tagid'].'&amp;ops='.$sess['hash'].'"><img src="'.ADMPATH.'/template/skin/'.$sess['skin'].'/images/del.png" alt="'.$lang['all_delet'].'" /></a>
							</td>
						</tr>';
			}
			echo '		<tr class="tfoot">
