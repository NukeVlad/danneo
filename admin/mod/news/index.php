	...
	if ($ADMIN_PERM == 1 OR in_array($ADMIN_ID, $CHECK_ADMIN['admid']))
	{
		...
		if ($_REQUEST['dn'] == 'index')
		{
			...
			while ($itemset = $db->fetchrow($inqset))
			{
				...
				{
					echo	in_array($itemset['setname'], array('rec', 'rating', 'comact', 'addit', 'rss')) ? '<tr><th colspan="2"></th></tr>' : '';
					echo '	<tr>
		...
		if ($_REQUEST['dn'] == 'list')
		{
			global $db, $conf, $realmod, $ro, $selective, $catid, $nu, $p, $cat, $s, $l, $ajax, $filter, $fid;

			$template['breadcrumb'] = array
				(
					'<a href="'.ADMPATH.'/index.php?dn=index&amp;ops='.$sess['hash'].'">Рабочий стол</a>',
					'<a href="'.ADMPATH.'/index.php?dn=content&amp;ops='.$sess['hash'].'">'.$lang['all_content'].'</a>',
					'<a href="index.php?dn=index&amp;ops='.$sess['hash'].'">'.$modname[PERMISS].'</a>',
			...
			echo '			</td>
						...
						<tr>
							...
							<td>
								...
								<a class="side-button" href="javascript:$.personalupload(\''.$sess['hash'].'&amp;objdir=/'.PERMISS.'/article/\');">'.$lang['file_upload'].'</a>
			...
			echo '		<tr>
						...
						<tr>
							...
							<td>
								...
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/article/\',\'&amp;field[1]=image_thumb&amp;field[2]=image\')" value="'.$lang['filebrowser'].'" type="button" />
								<input class="side-button" onclick="javascript:$.quickupload(\''.$sess['hash'].'&amp;objdir=/'.PERMISS.'/article/\')" value="'.$lang['file_review'].'" type="button" />
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input class="side-button" onclick="javascript:$.filebrowser(\''.$sess['hash'].'\',\'/'.PERMISS.'/article/\',\'&amp;field[1]=image&amp;field[2]=image_thumb\')" value="'.$lang['filebrowser'].'" type="button" />
							</td>
						</tr>
						<tr>
		...
		if ($_REQUEST['dn'] == 'addsave')
		{
			global	$catid, $public, $stpublic, $unpublic, $title, $subtitle, $cpu, $customs, $descript, $keywords, $textshort, $textmore, $hits,
					$image, $image_thumb, $image_align, $image_alt, $textnotice, $acc, $group, $act, $tagword, $images,
					$author, $imp, $id, $new, $p, $nu;
		...
		{
			global	$id, $catid, $public, $stpublic, $unpublic, $title, $subtitle, $cpu, $customs, $descript, $keywords, $textshort, $textmore, $hits,
					$image, $image_thumb, $image_align, $image_alt, $textnotice, $acc, $group, $act, $tagword, $images,
					$author, $imp, $p, $cat, $nu, $s, $l, $fid;
		...
		/**
		 * Комментарии отдельной новости (сохранение)
		 ----------------------------------------------*/
		...
		/**
		 * Комментарии - Все новости (сохранение)
		 ------------------------------------------*/
		...
		if ($_REQUEST['dn'] == 'new')
		{
			...
			echo '	<div class="section">
					<form action="index.php" method="post">
					<table id="list" class="work">
						<caption>'.$modname[PERMISS].': '.$lang['added'].'</caption>';
			if ($db->numrows($inq) > 0)
			{
				echo '	<tr><td colspan="6">'.$amount.'</td></tr>
						<tr>
							<th class="work-no-sort al">'.$lang['all_name'].'</th>
							<th class="work-no-sort">'.$lang['all_cat_one'].'</th>
							<th class="work-no-sort">'.$lang['author'].'</th>
							<th class="work-no-sort">'.$lang['sys_date'].'</th>
							<th class="work-no-sort">'.$lang['sys_manage'].'</th>
				...
				while ($item = $db->fetchrow($inq))
				{
					...
					echo '	<tr class="list">
								<td class="al pw25"><a href="index.php?dn=newadd&amp;id='.$item['id'].'&amp;ops='.$sess['hash'].'">'.preparse_un($item['title']).'</a></td>
		...
		/**
		 * Массовое удаление добавленных новостей
		 -------------------------------------------*/
							...
							<table id="arr-work" class="work">
								...
								<tr class="tfoot">
									<td>
										...
										<input class="main-button" onclick="javascript:history.go(-1)" value="'.$lang['all_goback'].'" type="button" />
									</td>
								</tr>
							</table>
							</form>
		...
		/**
		 * Модерировать добавленную новость
		 ------------------------------------*/
		...
		/**
		 * Быстрое редактирование названия новости
		 -------------------------------------------*/
		...
		/**
		 * Быстрое редактирование названия новости (сохранение)
		 -------------------------------------------------------*/
		...
		/**
		 * Быстрое изменение категории новости
		 ----------------------------------------*/
		...
		/**
		 * Быстрое изменение категории новости (сохранение)
		 ----------------------------------------------------*/
		...
		/**
		 * Быстрое изменение даты новости
		 ----------------------------------*/
		...
		/**
		 * Быстрое изменение даты новости (сохранение)
		 -----------------------------------------------*/
