...
$bs = array(
	...
	'sort' => array(
		'lang'		=> 'all_sorting',
		'form'		=> 'select',
		'value'		=> array('id' => 'ID', 'title' => 'all_name', 'public' => 'all_data', 'hits' => 'all_hits', 'comments' => 'menu_comment', 'totalrating' => 'rating'),
		'default'	=> 'id'
	),
	'order' => array(
		'lang'		=> 'all_sorting',
...
if (
	...
	$inq = $db->query
			(
				"SELECT art.*, cat.* FROM ".$basepref."_".$bs['mod']." AS art
	...
	if (isset($inq) AND $db->numrows($inq) > 0)
	{
		...
		$tm->unmanule['comment'] = ($bs['comment'] == 'yes') ? $config[$bs['mod']]['comact'] : 'no';
		...
		while ($item = $db->fetchrow($inq))
		{
			...
			// Кол. комментариев
			$ins['count'] = ($config[$bs['mod']]['comact'] == 'yes' AND $bs['comment'] == 'yes') ? $item['comments'] : '';
