 ...
 * Глобальные
 */
global	$db, $basepref, $config, $lang, $usermain, $ro, $tm, $api, $userapi,
		$global, $to, $p, $c, $id, $ye, $mo, $da, $ccpu, $cpu;
...
if ($to == 'cat')
{
	...
	while ($citem = $db->fetchassoc($inq, $config['cache']))
	{
		$area[$citem['parentid']][$citem['catid']] = $menu[$citem['catid']] = $obj['id'][$citem['catid']] = $obj['ccpu'][$citem['catcpu']] = $citem;
	}
...
if ($to == 'page')
{
	...
	if (isset($p)) {
		if ($p >= 1 AND $c == 1) {
			$seopage = ', '.mb_strtolower($lang['page_one']).'-'.$p;
		} else {
			$seopage = ', '.mb_strtolower($lang['page_one']).'-'.$c;
		}
	}

	$c = preparse($c, THIS_INT);
	$p = preparse($p, THIS_INT);
	$p = ($p <= 1) ? 1 : $p;
	...
	/**
	 * Ошибка листинга разбивки страницы
	 */
	$count_pages = explode("[page]", $item['textshort'].$item['textmore']);
	if ($p > count($count_pages))
	{
		$tm->noexistprint();
	}
	...
	if ($conf['comact'] == 'yes')
	{
		...
		// Вывод
		if ($item['comments'] > 0) {
			$ins['comment'] = $cm->comment($item['id'], $item['comments'], $ins['cpu'], $ins['catcpu'], $c, true);
		}
	...
	$tm->unmanule['ajax']   = $config['ajax'];
	$tm->unmanule['rating'] = $conf['rating'];
	$tm->unmanule['social'] = $config['social_bookmark'];
	$tm->unmanule['redate'] = ($conf['date'] == 'yes' AND $item['uppublic'] > 0) ? 'yes' : 'no';
	$tm->unmanule['files']  = ( ! empty($item['files']) AND $facc AND $fgroups) ? 'yes' : 'no';
	 ...
	 * Видео
	 */
	$ins['temtlate_video'] = $tm->parsein($tm->create('mod/'.WORKMOD.'/video'));
	if ( ! empty($item['video']))
	{
		$ins['path_video'] = (substr($item['video'], 0, 7) == 'http://') ? $item['video'] : SITE_URL.'/'.$item['video'];
		$ins['vlocal'] = $tm->parse(array('video' => $ins['path_video']), $tm->manuale['vlocal']);
	}
	if ( ! empty($item['youtu']))
	{
		$ins['vtube'] = $tm->parse(array('youtu' => $item['youtu']), $tm->manuale['vtube']);
	}
	$ins['video'] = $tm->parse(array
		(
			'vlocal'  => $ins['vlocal'],
			'vtube'  => $ins['vtube']
		),
	 ...
	 */
	$ins['full_content'] = $ins['textshort'].$ins['textmore'];
	$ins['explode'] = explode("[page]", $ins['full_content']);
	$ins['counts'] = count($ins['explode']);
	$ins['contents'] = $ins['explode'][$p - 1];
	...
	if ($ins['counts'] > 0)
	{
		$ins['link'] = 'index.php?dn='.WORKMOD.$ins['catcpu'].'&amp;to=page&amp;id='.$item['id'].$ins['cpu'];

		// Рубрикатор в начале
		if (preg_match("/\[pagename:([\pL\pNd\pZs\pP\pM\|]*)\]/u", $ins['full_content'], $ins['attr']))
		{
			$ins['pagination'] = $tm->parsein($tm->create('mod/'.WORKMOD.'/pagination'));

			$ins['work'] = explode('|', $ins['attr'][1]);
			$ins['count'] = sizeof($ins['work']);

			$ins['lrows'] = null;
			$com = (isset($c) AND ! empty($c)) ? '&amp;c='.$c : '';
			foreach ($ins['work'] as $k => $v)
			{
				if ($ins['counts'] == $ins['count'])
				{
					if (isset($ins['explode'][$k]) AND isset($ins['work'][$k]))
					{
						if (($k + 1) == 1)
						{
							if ($p == ($k + 1)) {
								$ins['lrows'] .= $tm->parse(array('title' => $ins['work'][$k]), $tm->manuale['title']);
							} else {
								$ins['lrows'] .= $tm->parse(array('title' => $ins['work'][$k], 'link' => $ro->seo($ins['link'].'&amp;p=1'.$com)), $tm->manuale['links']);
							}
						}
						else
						{
							if ($p == ($k + 1)) {
								...
								$ins['lrows'] .= $tm->parse(array('title' => $ins['work'][$k], 'link' => $ro->seo($ins['link'].'&amp;p='.($k + 1).$com)), $tm->manuale['links']);
							}
						}
					}
				}
			...
			// Вывод рубрик
			$ins['rubric'] = $tm->parse(array('rubricrows' => $ins['lrows']), $ins['pagination']);
		}

		// Содержимое после обработки
		$ins['contents'] = preg_replace("/\[(page|pagename:([\pL\pNd\pZs\pP\pM\|]*))\]/u", '', $ins['contents']);
	}

	// Листинг разбивки, внизу страницы
	if ($ins['counts'] > 1)
	{
		$pages = $api->pages('', '', 'index', WORKMOD.$ins['catcpu'].'&amp;to=page&amp;id='.$item['id'].$ins['cpu'], 1, $p, $ins['counts']);
		$ins['pages'] = $tm->parse(array('text'  => $lang['all_pages'], 'pages' => $pages), $tm->manuale['pagesout']);
	}
	...
	$tm->parseprint(array
		(
			...
			'image'		=> $ins['image'],
			'rubric'	=> $ins['rubric'],
			'contents'	=> $api->siteuni($ins['contents']),
			'pages'		=> $ins['pages'],
