...
$global['insert']['breadcrumb'] = array('<a href="'.$ro->seo('index.php?dn='.WORKMOD).'">'.$global['modname'].'</a>', $lang['isset_error']);
...
if ($id > 0)
{
	$obj = $db->fetchrow($db->query("SELECT seaid, seaword FROM ".$basepref."_".WORKMOD."_search WHERE seaid = '".$id."' AND seaip = '".$db->escape(REMOTE_ADDRS)."'"));
}
else
{
	// Проверяем на флуд
...
{
	$tm->message($lang['following_no_found'], 0);
}
else
{
		...
		while ($item = $db->fetchrow($inq))
		{
			...
			$ins['content'][] = $tm->parse(array
				(
					...
					'desc'		=> $tm->wordlight($obj['seaword'], $api->siteuni($item['textshort'])),
					'img'		=> $ins['image'],
					'url'		=> $ins['url'],
					'author'	=> $ins['author'],
	...
	{
		$tm->message($lang['following_no_found'], 0, 1, 1);
	}
