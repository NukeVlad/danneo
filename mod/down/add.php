...
global	$dn, $to, $db, $basepref, $config, $lang, $usermain, $tm, $api,
		$global, $catid, $title, $text, $image_thumb, $image, $catcache, $selective, $cid, $respon;
 ...
 * Меню, хлебные крошки
 */
$global['insert']['current'] = $lang['down_add'];
$global['insert']['breadcrumb'] = '
	<a href="'.$ro->seo('index.php?dn='.WORKMOD).'">'.$global['modname'].'</a> <i>&#187;</i>
	<a href="'.$ro->seo('index.php?dn='.WORKMOD.'&amp;re=add').'">'.$lang['down_add'].'</a>';
...
if ($to == 'index')
{
	 ...
	 */
	$global['insert']['current'] = $lang['down_add'];
	$global['insert']['breadcrumb'] = '<a href="'.$ro->seo('index.php?dn='.WORKMOD).'">'.$global['modname'].'</a> <i>&#187;</i> '.$lang['down_add'];
...
if ($to == 'save')
{
	 ...
	 * Данные
	 */
	if ( ! empty($title) AND ! empty($textshort) AND ! empty($url))
	{
		$ins['title'] = $api->sitesp(preparse($title, THIS_TRIM));
		$ins['subtitle'] = $ins['title'];
		$ins['customs'] = $ins['title'];
		$ins['descript'] = $ins['title'];
		$ins['textshort'] = preparse($textshort, THIS_TRIM);
		$ins['textmore'] = preparse($textmore, THIS_TRIM);
		$ins['keywords'] = $api->seokeywords($ins['title'].' '.$ins['textshort'].' '.$ins['textmore'], 5, 35);

		$ins['url'] = $userapi->addurl($url);

		$trl = new Translit();
		$cpu = $trl->title($trl->process($ins['title']));

		// check dub
		check_name('cpu', $cpu);
		check_name('title', $ins['title']);
		check_title($ins['title']);
		...
		if ($ins['active'])
		{
				...
				$db->query
					(
						"INSERT INTO ".$basepref."_".WORKMOD." VALUES (
						 ...
						 '0',
						 '".$db->escape($cpu)."',
						 '".$db->escape($api->sitesp($ins['customs']))."',
						 '".$ins['url']."',
						 '0',
						 '".$db->escape($api->sitesp($ins['descript']))."',
						 '".$db->escape($api->sitesp($ins['keywords']))."',
						 '".$db->escape($api->sitesp($ins['title']))."',
						 '".$db->escape($api->sitesp($ins['subtitle']))."',
						 '".$db->escape($ins['textshort'])."',
						 '".$db->escape($ins['textmore'])."',
						 '',
						 '',
						 '',
						 '".$usermain['uname']."',
			...
			// Сообщение, ОК
			$tm->message($lang['down_add_public'], 0, 0);
		}
		else
		{
			$db->query
		...
		if ($conf['mailadd'] == 'yes')
		{
			$subject = $global['modname'].': '.$lang['down_subject'];
		...
		// Сообщение, ОК
		$tm->message($lang['down_add_text'], 0, 0);
	}
	else
	{
