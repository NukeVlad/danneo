...
$valid = $db->query
			(
				"SELECT * FROM ".$basepref."_".WORKMOD." WHERE act = 'yes'
				 AND finish > '".NEWTIME."'
				 AND id = '".$id."'"
			);
...
{
	if ($item['acc'] == 'user' OR $conf['comwho'] == 'user')
	{
...
if ($in)
{
	...
	if ($ajax == 0)
	{
		...
		// url
		$ins['url'] = $ro->seo('index.php?dn='.WORKMOD.'&amp;to=page&amp;id='.$item['id'].$ins['cpu'].$ins['pc']);
