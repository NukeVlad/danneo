...
$inq = $db->query
		(
			"SELECT a.id, a.catid, a.public, a.stpublic, a.unpublic, a.cpu, a.title, a.text, a.image_thumb, a.image_alt, a.act, b.catname, b.catcpu, b.rss
			 FROM ".$basepref."_photos AS a
			 LEFT JOIN ".$basepref."_photos_cat AS b ON (a.catid = b.catid)
...
{
	$ai[$item['id']]['pubdate'] = $item['public'];
	...
	$ai[$item['id']]['description'] = $api->siteuni(strip_tags($item['text']));
	...
	if ($global['full'])
	{
		$ai[$item['id']]['yandex'] = $api->siteuni(strip_tags($item['text']));
	}
	$i++;
}
