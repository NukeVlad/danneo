/*!
 * jQuery UI - v1.10.3 - 2013-11-30  http://jqueryui.com
...
$( function() {
	...
	var bmenu = false;
	var cmenu = cookie.get("bmenu");

	if (cmenu !== undefined && cmenu != "false") {
		var bmenu = parseInt(cmenu);
    }
	$(".js-block-menu").accordion({
		active: bmenu,
	...
	$(".js-block-menu > li a").click( function() {
		var bvalue = $(".js-block-menu").accordion("option", "active");
		cookie.set("bmenu", bvalue ,
						{
							expires: date,
							path: "/",
							domain: $.host,
	...
	$('.js-block-menu > li strong > span').remove();
	
	$(".js-block-menu > li").click( function() {
		var link = $(this).find('a').attr('href');
		var list = $(this).find('strong');
		if (list.length > 0) {
			window.location;
		} else {
	...
	$(".js-block-menu > li").each( function() {
		var list = $(this).find('li');
		if (list.length > 0) {
			$($(this).find('a')[0]).attr("href", "javascript:;");
		} else {
			$($(this).find('a > span')[0]).remove();
		}
	});
});
...
(function($)
{
	$.fn.jsmenu = function()
	{
		...
		var tmenu = false;
		var ctmenu = cookie.get("tmenu");

		if (ctmenu !== undefined && ctmenu != "false") {
			var tmenu = parseInt(ctmenu);
		}
		$(".js-menu").accordion({
			active: tmenu,
			header: "> li > :first-child,> :not(li):even",
		...
		$(".js-menu > li a").click( function() {
			var bvalue = $(".js-menu").accordion("option", "active");
			cookie.set("tmenu", bvalue ,
						{
							expires: date,
							path: "/",
							domain: $.host,
